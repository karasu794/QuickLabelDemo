...
export default function AddressBookPicker({ role, onSelect, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const res = await fetch('/api/address-book', { credentials: 'include' })
        if (!res.ok) {
          if (res.status === 401) {
            toast.error('ログインが必要です')
            throw new Error('ログインが必要です')
          }
          throw new Error(`Failed to load address book: ${res.status}`)
        }
        const data = await res.json()
        const items = Array.isArray(data) ? data : []
        const mapped: AddressDTO[] = items.map((r: any) => ({
          name: r.contact_name || '',
      ...
      <Card className="relative z-10 w=[90vw] max-w-2xl max-h-[80vh] overflow-hidden">
        <CardHeader className="px-5 py-4">
          <div className="flex items-center justify-between">
            <CardTitle className="text-base md:text-lg">保存した宛先から入力</CardTitle>
        ...
        <CardContent className="px-5 pb-5 space-y-4">
          ...
          {items && items.length === 0 && !error && (
            <div className="text-sm text-gray-600" data-test="empty-addressbook">保存済みの宛先が見つかりません</div>
          )}
