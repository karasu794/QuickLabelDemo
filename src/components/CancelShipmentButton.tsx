...
export default function CancelShipmentButton({
  ...
  // キャンセル不可能なステータスをチェック
  const isCancellable = currentStatus !== 'CANCELLED' && 
  ...
  const handleCancel = async () => {
    ...
    const confirmCancel = window.confirm(
      `追跡番号 ${trackingNumber} の発送をキャンセルし、返金処理を行いますか？\n\nこの操作は取り消すことができません。`
    )
    ...
    try {
      console.log('🚫 発送キャンセル・返金処理開始:', trackingNumber)
      
      // Server Actionを呼び出し
      const response = await cancelShipmentAction(trackingNumber, squarePaymentId)
      
      if (response.success) {
        console.log('✅ キャンセル・返金成功:', response.message)
        setResult({
          type: 'success',
      ...
      console.error('❌ キャンセル・返金処理エラー:', error)
      setResult({
        type: 'error',
        message: 'キャンセル・返金処理中にエラーが発生しました'
      })
    } finally {
      setIsLoading(false)
    }
