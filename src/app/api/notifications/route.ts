  ...
  try {
    ...
    if (!id || typeof id !== 'string') {
      return NextResponse.json(
        { error: 'BAD_REQUEST', message: '通知IDが必要です' },
        { status: 400 }
      )
    }
    ...
    if (action !== 'mark_as_read') {
      return NextResponse.json(
        { error: 'BAD_REQUEST', message: '無効なアクションです' },
        { status: 400 }
      )
    }
    ...
    if (!Number.isFinite(idNum)) {
      return NextResponse.json(
        { error: 'BAD_REQUEST', message: '無効な通知IDです' },
        { status: 400 }
      )
    }
    ...
    if (error) {
      ...
      return NextResponse.json(
        { error: 'INTERNAL', message: '通知の更新に失敗しました' },
        { status: 500 }
      )
    }
    ...
    if (!data) {
      return NextResponse.json(
        { error: 'NOT_FOUND', message: '指定された通知が見つかりません' },
        { status: 404 }
      )
    }
    ...
    console.error('通知更新エラー:', error)
    return NextResponse.json(
      { error: 'INTERNAL', message: 'サーバーエラーが発生しました' },
      { status: 500 }
    )
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!type || typeof type !== 'string') {
      return NextResponse.json(
        { error: 'BAD_REQUEST', message: '通知タイプが必要です' },
        { status: 400 }
      )
    }
    ...
    if (!message || typeof message !== 'string') {
      return NextResponse.json(
        { error: 'BAD_REQUEST', message: 'メッセージが必要です' },
        { status: 400 }
      )
    }
    ...
    if (error) {
      ...
      return NextResponse.json(
        { error: 'INTERNAL', message: '通知の作成に失敗しました' },
        { status: 500 }
      )
    }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json(
        { error: 'INTERNAL', message: '通知の削除に失敗しました' },
        { status: 500 }
      )
    }
