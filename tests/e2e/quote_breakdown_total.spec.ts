...
test.describe('見積もり内訳合計検証', () => {
  test('declaredValue=0 → 内訳合計=合計', async ({ page, context, baseURL }) => {
    ...
    const additionalHandlingEl = page.locator('[data-test="breakdown-row-additionalHandlingSurcharge"]')
    ...
    const additionalHandling = await extractAmount(additionalHandlingEl)
